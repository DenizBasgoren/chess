...
document.getElementById("startgame").onclick = function () {
	if (document.getElementById("whitename").value && document.getElementById("blackname").value) {
		...
		document.getElementById("blacknamee").innerHTML = document.getElementById("blackname").value.toUpperCase().slice(0,20);
...
function moveAttempt(x, y, fx, fy) {
	...
	x = Number.parseInt(x);
	y = Number.parseInt(y);
	fx = Number.parseInt(fx);
	fy = Number.parseInt(fy);
...
function debug2() {
	...
	var x = Number.parseInt(selected[3]);
	var y = Number.parseInt(selected[1]);
	var valids = [];
...
document.getElementById("endgame").onclick = function () {
	...
	touchcoords[0]=0;
	touchcoords[1]=0;
	document.getElementById("p"+touchcoords[1]+"p"+touchcoords[0]).style["background-color"] = ((i%2)+(j%2)==1 ? '#750' : '#ee9');
...
function drawstuff () {
	...
	for (var i = 1; i<=8; i++) {
		children += "<tr>\n";
		for (var j = 1; j<=8; j++) {
			children += '<td id="p'+i+'p'+j+'" ondragover="allowDrop(event)" ondrop="drop(event)" ontouchend="touched(event)" style="width: 50px; height: 50px;  border: 1px solid black; background-color: #'+ ((i%2)+(j%2)==1 ? '750' : 'ee9') + '"></td>\n'; // 1c6 4f9 // 750 ee9
		}
		children += "</tr>\n";
	}
...
function touched(event) {
	...
	if (touchcoords[0]==0 ) {
		if (board[event.target.id[1]][event.target.id[3]]=='b') {return; }
