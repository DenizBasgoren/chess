...
function cputime() {
	...
	var moveableareas = [];
		...
		/*
		After finding all legal moves, computer searches if it can capture a piece. If yes, the most precious one is selected and captured.
		If no pieces can be captured, cpu looks if it can check the king. If yes, checks.
		If no, makes a move randomly
		*/

		var edible = [];


		// an inner function returning priority of a target piece
		//0=empty 1=pawn 2=knight 3=bishop 4=rook 5=queen
		...
		// If there are no pieces that can be captured, search for check
		if (maxpriority == 0) {
		...
		// If there are  pieces to capture. 
		if (edible.length==0) {
			for (i = 0; i<moveableareas.length; i++) {
